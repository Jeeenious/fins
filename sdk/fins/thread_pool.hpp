...
//   实时优先级：worker 同时提升为 SCHED_FIFO（set_realtime，默认 prio 50）——同核 CFS 线程
//   （本进程未绑核的主线程/计时线程，可能被 wake-affine 放进 worker 核）无法抢占 RT worker，
//   忙等待自旋不被进程内线程打断；设 RT 需 CAP_SYS_NICE（WSL 默认 root OK），失败 warn 降级。
