...
//   每个 worker 的自调度循环：
//     while (!stop_.load()) { if (!cb_) break; if (!cb_()) break; }
...
namespace fins::rt {
  ...
  private:
    ...
    /// 返回 bool：true 继续取下一任务；false = 无更多工作（停止）→ worker 退出循环。
  ...
  public:
    ...
    /// 返回 true 继续取下一任务；返回 false = 无更多工作（停止）→ worker 退出循环。
