...
回绕与释放节拍（2026-09-04 定稿，见 `g_state.hpp::rollover_hp`）：

- `rollover_hp` 仍由“本超周期完工”（`is_hp_done` = done_count==dag.size）触发——干完即翻页，只重置
  done/ready/pred_left/tp 游标，不清顶点；但**超周期起点不再拨回完工时刻**，而是推进到绝对网格上的
  **下一未来边界**（起点按 H 整拍对齐 expand 起点，`hyper_start += ceil(delta/H)·H`，delta=完工距起点）。
- 于是**周期任务只在真实周期锚点（起点+offset）释放**：早排空 → 空等到下一边界（不提前放下一拍，
  单源 period=100 实测释放间隔=100ms）；排空晚于边界（过载）→ **跳过已错过的整拍、对齐未来边界**
  并告警（不累积漂移）。
- **非周期（事件）任务不受影响**：绑定前序完成即就绪即跑（实测事件跟随的滞后 = 前级忙等时长）。
- 修正了旧行为“完工即回绕把起点拨到完工时刻 → offset=0 的 tp 立即重放 → 周期任务背靠背自触发
  （单任务实测间隔≈忙等时长、利用率趋近 1）”。

拓扑合法性（`check_topology`，第二级校验；违反 → apply 被拒并报错，不进入运行）：

- 无输入节点（数据流源）**必填显式 period**；
- 每个**非 loop 输入端口必须有生产者**——拒绝“孤立输入”伪根：只声明输入却无人产出该端口的节点
  会退化成根（无 tp、无前序、无继承周期、N=1），expand 时只跑一次且 rollover 永不重放，还会让周期
  兄弟节点因它永不完成而无法翻页。loop(自反馈)端口例外，producer = 节点自身。
- 两条合起来 ⇒ **任何合法的事件节点都必然位于一条以显式周期源为根的链上**：要么时间触发（周期），
  要么由前级数据完成驱动（事件跟随），不存在“无源可自激活的非周期节点”。
